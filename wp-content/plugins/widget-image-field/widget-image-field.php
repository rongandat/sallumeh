...
class WidgetImageField
{
    ...
    function get_widget_field( $field_name = null,$type=null )
    {
        $field = false;
        $image=$type=='main'?$this->widget->image_field:$this->widget->image_bg_field;
        if( $this->widget && ( isset( $image ) || $field_name ) )
        {
            $field  = "<div class='iti-image-widget-field'><div class='iti-image-widget-image' id='" . $this->widget->id . "'>";
            $field .= "<input type='hidden' style='display:none;' id='" . $this->widget->get_field_id( $image ) . "' name='" . $this->widget->get_field_name( $image ) . "' value='" . $this->image_id . "' />";
